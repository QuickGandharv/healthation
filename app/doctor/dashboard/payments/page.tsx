...
import { Column, DataTable, FilterOption } from "@/components/custom/data-table";
import { useMyTransactions } from "@/queries/common/useMyTransactions";
import type { TransactionItem } from "@/types/common/transactions";
...
export default function PaymentsPage() {
    ...
    const { data, isLoading, isError, error } = useMyTransactions();
    console.log("data", data);

    const mapPaymentMethod = (
        paymentMethod?: string
    ): "card" | "upi" | "cash" | "pending" => {
    ...
    const payments: Payment[] = useMemo(() => {
        const transactions: TransactionItem[] = data?.data ?? [];
        ...
        return transactions.map((item) => ({
            ...
            status: mapPaymentStatus(item.status_label, item.status),
            razorpayId: item.transaction_id ?? "-",
    ...
    const columns: Column<Payment>[] = [
        ...
        },
        {
            ...
            cell: (payment) => (
                <Badge
                    variant="outline"
                    className={`flex items-center gap-1 ${getMethodColor(payment.method)}`}
                >
                    {getMethodIcon(payment.method)}
                    <span className="capitalize">{payment.method}</span>
                </Badge>
        ...
        },
        {
            ...
            cell: (payment) => (
                <span className="font-mono text-xs">
                    {payment.appointmentId?.length > 12
    ...
    const filterComponents = {
        ...
        status: (value: string) => <span>Status: {value}</span>,
    };
    ...
    return (
        <div className="space-y-6">
            ...
            <DataTable
                ...
                columns={columns}
                filters={filters}
                searchPlaceholder="Search by patient, doctor, transaction ID, or order ID..."
                onSearch={setSearchQuery}
                onFilter={handleFilter}
                onClearFilters={handleClearFilters}
                activeFilters={{
